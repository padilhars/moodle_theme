...
if ($hassiteconfig) {
    $page = new admin_settingpage('themesettingufpel', get_string('pluginname', 'theme_ufpel'),
        'theme/ufpel:configure'); // Usa capability própria para configurar o tema.

    // Título/descricao padrão (choosereadme é exibido no seletor de temas).
    $page->add(new admin_setting_heading('theme_ufpel_general', get_string('generalsettings', 'theme_ufpel'), ''));
    ...
    // Adicione à página de configurações do tema (dentro do if ($hassiteconfig) / admin page).
